...
% if what_u_want == 'integration':
    % for derivative_order in reversed(range(dependent_vars_count)):
        ...
        ${curr_var}_next = ${curr_var} + ${curr_var}_diff(idx, ${get_dep_indep_params_string(dep=True, indep=True, type_prefix=False, next_postfix=False)}) * d${list(sde.row_iterator('type', ['independent variable']))[0].Index};
    % endfor
